...
public class AckMessage<T> extends BaseAddrServerMessage<T> {
     ...
     /**
      * Creates an ACK message from the PRIMARY server directed to a CLIENT
      * when no ChatServer is available.
      *
      * @param senderPID The process ID of the PRIMARY server sending this message
      * @return An {@code AckMessage} with a "no host found" payload
      */
     public static AckMessage<String> noChatHost(long senderPID) {
          return new AckMessage<>(AckObjectTypes.NOHOST, senderPID, Roles.PRIMARY, Roles.CLIENT,
                    "404 ChatServer Not Found — they ghosted you.");
     }

     /**
      * Creates an ACK message from the PRIMARY server directed to a REPLICA.
      * Indicates the REPLICA was successfully registered.
      *
      * @param senderPID The process ID of the PRIMARY server sending this message
      * @param payload   The assigned PID to be sent as confirmation
      * @return An {@code AckMessage} ready to be sent from PRIMARY to REPLICA
      */
     ...
     /**
      * Creates an ACK message from the PRIMARY server directed to a CHATSERVER.
      * Indicates the ChatServer was successfully registered.
      *
      * @param senderPID The process ID of the PRIMARY server sending this message
      * @param payload   The assigned PID to be sent as confirmation
      * @return An {@code AckMessage} ready to be sent from PRIMARY to CHATSERVER
      */
