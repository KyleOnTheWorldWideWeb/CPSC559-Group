...
public class ServerFailureMessage<T> extends BaseAddrServerMessage<T> {
    ...
    /**
     * Constructs a new {@code ServerFailureMessage}, typically used within the Addressing Server module
     * to notify peers of a detected failure in the network.
     * <p>
     * This overload accepts a unique {@code messageID}, allowing it to be tracked and acknowledged
     * as part of the sequential consistency mechanism between the PRIMARY and its REPLICA processes.
     * It is strongly recommended to generate this ID using the {@link MessageIDGenerator} to ensure uniqueness
     ...
     * @param objectType  A string representing the type of failure event (e.g., "HeartbeatTimeout", "ChannelClosed").
     * @param senderPID   The process ID of the sender issuing the failure notification.
     * @param senderRole  The role of the sender (e.g., PRIMARY, REPLICA, CHATSERVER).
     * @param targetRole  The intended recipient's role (e.g., REPLICA, CHATSERVER).
     * @param payload     A descriptive payload (typically a {@code Long} representing the failed server's PID or
     *                    other relevant context). Must be interpreted by the receiver.
     */
    public ServerFailureMessage(long messageID, String objectType, long senderPID, String senderRole, String targetRole, T payload) {
     ...
     *
     * <p>
     * This factory method generates a {@code ServerFailureMessage} with the object type set to
     ...
     * @return a {@code ServerFailureMessage<Long>} encapsulating the failure event for a ChatServer
     */
    public static ServerFailureMessage<Long> chatServerFailed(long messageID, long senderPID, String senderRole, String receiverRole, Long failedPID) {
        return new ServerFailureMessage<>(messageID, ObjectTypes.CHATSERVER_FAILURE, senderPID, senderRole, receiverRole, failedPID);
    }
     ...
     * @return a {@code ServerFailureMessage<Long>} encapsulating the failure event for an AddressingServer
     */
    public static ServerFailureMessage<Long> addrServerFailed(long messageID, long senderPID, String senderRole, String receiverRole, Long failedPID) {
        return new ServerFailureMessage<>(messageID, ObjectTypes.ADDRSERVER_FAILURE,senderPID, senderRole, receiverRole, failedPID);
    }
