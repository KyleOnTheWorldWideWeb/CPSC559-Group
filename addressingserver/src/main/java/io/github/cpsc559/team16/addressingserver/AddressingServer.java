...
public class AddressingServer {
     ...
     * This should be the port the chat server used to register itself with the
     * Addressing Server.
     */
     ...
     * <ul>
     * <li>PRIMARY - the leader process in charge of coordinating connections in the
     * network.</li>
    ...
    /**
     * A mapping of unique chat server IDs to their corresponding
     * {@link ChatServerInfo} records.
     * <p>
     * A {@code ChatServerInfo} record is maintained and updated by the Primary
     * Addressing Server
     ...
     * <p>
     * All AddressingServer processes will have
     * {@code ChatServerInfo.maxClientCount} = 0
     * </p>
     */
    private ArrayList<AddrServerInfo> replicaRecords;

    /**
     * Contains the set of all open channels between the Primary
     * {@code AddressingServer} and its Replicas.
     * <p>
     * The implementation of our push protocol for consistency and leader election
     * for fault tolerance is such that
     * every server in the data structure will be used during updates and elections,
     * so a simple DS (array) is ideal.
     * </p>
     */
    ...
    /**
     * Constructs an AddressingServer with the specified host address, client port,
     * peer port, and chat server port.
     * <p>
     * This constructor initializes a new, empty address log to track registered
     * chat servers.
     * </p>
     *
    ...
    public AddressingServer() {
        ...
        // TODO - Figure out how to get the outward facing IP address (needed for the
        // primary AS when it starts up)
     ...
     * <p>
     * This method updates the AddressingServer's registry of chat servers by
     * replacing its internal
     * address log with the new map passed as a parameter. This allows for dynamic
     * reconfiguration or recovery
     * by resetting the chat server records.
     * </p>
     *
     * @param newChatServerRecords a {@code Map} of chat server IDs to
     *                             {@link ChatServerInfo} objects representing the
     *                             new address log.
     */
    ...
    // /**
    // * Replaces the current address log with the one received from a remote
    // server.
    // * <p>
    // * The new address log is transmitted over the network, deserialized, and then
    // assigned by reference.
    // * This ensures that the server uses the most up-to-date address log
    // information.
    // * </p>
    // *
    // * @param newAddressLog the address log object received from the network
    // */
    // public void updateAddressLogFromNetwork(Map<Long,ChatServerInfo>
    // newAddressLog) {
    //
    // }

    /**
     * Generates a unique identifier for a chat server by incrementing the internal
     * counter.
     *
    ...
    /**
     * Deregisters a chat server by removing it from the {@code addressLog}
     * containing all known chat servers.
     *
     * @param chatServerID the unique identifier of the chat server to be
     *                     deregistered.
     * @return {@code true} if the server was successfully removed; {@code false}
     *         otherwise.
     */
     ...
     *
     * @return an {@link Optional} containing the active host address in the format
     ...
     * // TODO - Implement methods for all three channels in the 559Project -
    ...
    // this.chatServerPort = ((InetSocketAddress)
    // channel.getLocalAddress()).getPort();
    // System.out.println("Listening for Chat Servers on Port: " +
    // this.chatServerPort);
     ...
     * <p>
     * This method calls {@link #getActiveHost()} to determine the active chat
     * server address.
     * If an active host is found, its address (formatted as
     * "hostAddress:clientPort") is sent to the client.
     * Otherwise, a message indicating no active host is available is sent.
     * </p>
     * <p>
     * Since the connection does not need to persist in either case, the message is
     * sent, and the channel is closed.
     * </p>
     * 
     * @param newClientChannel the {@link SocketChannel} representing the client
     *                         connection.
     * @throws IOException if an I/O error occurs while writing to or closing the
     *                     channel.
     *
    ...
    /**
     * Creates a record for a chat server by generating a unique ID and inserting
     * its
     * ChatServerInfo record into the global addressLog.
     * <p>
     * This method generates a unique ID using {@code generatePID()}, creates a new
     * {@link ChatServerInfo} object with the given parameters, and then inserts it
     * into the addressLog. If a record
     * with the same ID already exists, it logs a warning message (and overwrites
     * it).
     * </p>
     *
     ...
     * @param maxClientCount  the maximum number of client connections allowed for
     *                        this server.
     *
     * @return serverPID The unique process id generated for the newly registered
     *         {@code ChatServer}
     */
    private long createChatServerRecord(String chatHostAddress, int addrServerPort, int chatClientPort,
        ...
        try {
            ...
            ChatServerInfo newServer = new ChatServerInfo(serverPID, chatHostAddress, addrServerPort, chatPeerPort,
                    chatClientPort, maxClientCount);
            ...
            if (previous != null) {
                System.err.println(
                        "WARNING: A server with ID " + serverPID + " already existed. Overwriting existing entry.");
                // TODO: add logic to skip a range of ID values and re-insert the overwritten
                // record - "previous"
            } else {
    ...
    /**
     * Registers a chat server by retrieving its IP address from the provided
     * SocketChannel
     * and passing that IP along with port numbers and maximum client count to
     * create a chat server record.
     * <p>
     * Responds to the chat server with an ACK in the form of it's newly assigned
     * process ID.
     * </p>
     *
     * @param newChatServerChannel the {@link SocketChannel} representing the
     *                             incoming connection from a chat server.
     * @throws IOException if an I/O error occurs while obtaining the remote
     *                     address.
     */
    private void registerChatServer(SocketChannel newChatServerChannel) throws IOException {
        // Retrieving the remote address from the SocketChannel and casting it to
        // InetSocketAddress.
        ...
        // The chat server should be telling the AddressingServer what ports its using
        // and its max connections.
        try {
            ...
            while (true) {
                int bytesRead = newChatServerChannel.read(buffer);
                if (bytesRead == -1)
                ...
                buffer.get(data);
                jsonStringBuilder.append(new String(data, StandardCharsets.UTF_8));
                buffer.clear();

                // Check if message contains a newline (end of JSON)
            ...
            if (jsonString.isEmpty()) {
                System.err.println("Received empty JSON. Connection closed prematurely.");
                return;
            }

            System.out.println("Received registration request: " + jsonString);

            // Parse JSON safely
            JSONObject requestData = new JSONObject(jsonString);

            String hostAddress = requestData.getString("hostAddress");
            int addrServerPort = requestData.getInt("addrServerPort");
            int chatServerPort = requestData.getInt("chatServerPort");
            int chatPeerPort = requestData.getInt("chatPeerPort");
            int maxClients = requestData.getInt("maxClients");

            long pid = createChatServerRecord(hostAddress, addrServerPort, chatServerPort, chatPeerPort, maxClients);
            String response = pid + "\n";

            // Send response
            ByteBuffer responseBuffer = ByteBuffer.wrap(response.getBytes(StandardCharsets.UTF_8));
            while (responseBuffer.hasRemaining()) {
                newChatServerChannel.write(responseBuffer);
            }

            newChatServerChannel.socket().getOutputStream().flush();

            System.out.println("Sent response to chat server: " + response);

            // **Send all chat server information to the newly joined chat server**
            sendAllChatServersInfo(newChatServerChannel);

        } catch (Exception e) {
            System.err.println("Failed during chat server registration with address: " + chatServerHostAddr);
            e.printStackTrace();
        } finally {
            // newChatServerChannel.shutdownOutput();
            newChatServerChannel.close();
        }
    }

    /**
     * Sends all registered chat server information as a JSON array.
     * This method is useful for clients looking to get a list of all available chat
     * servers.
     *
     * @param requestChannel The SocketChannel to send data to.
     */
    private void sendAllChatServersInfo(SocketChannel requestChannel) {
        try {
            // Build JSON array of all chat server details
            JSONObject response = new JSONObject();
            response.put("chatServers", chatServerRecords.values().stream().map(server -> {
                JSONObject jsonServer = new JSONObject();
                jsonServer.put("pid", server.getPID());
                jsonServer.put("hostAddress", server.getHostAddress());
                jsonServer.put("peerPort", server.getPeerPort());
                jsonServer.put("status", server.getStatus().toString());
                return jsonServer;
            }).toArray());
            ...
            ByteBuffer buffer = ByteBuffer.wrap(jsonString.getBytes(StandardCharsets.UTF_8));
            while (buffer.hasRemaining()) {
                requestChannel.write(buffer);
            }

            System.out.println("Sent all chat server info to the new server: " + jsonString);
            requestChannel.shutdownOutput();

        } catch (IOException e) {
            System.err.println("Error sending chat server information: " + e.getMessage());
            e.printStackTrace();
        }
    }
     ...
     * @throws IOException if an I/O error occurs while opening or binding the
     *                     channel
     */
    ...
    /**
     * Initializes the AddressingServer network access by binding all required NIO
     * channels.
    ...
    public void mainEventLoop() throws IOException {
        while (true) {
             ...
             * Here, we iterate through all keys (channels) that are "ready" for an I/O
             * operation.
             * We know that at least one such key exists because of the preceding
             * `selector.select()` method invocation.
             */
            ...
            while (keys.hasNext()) { // Loop until there are no more keys (channels with I/O events).
                ...
                /*
                 * Establishing a new connection. `isAcceptable` returns true if a
                 * ServerSocketChannel registered
                 * with `selector` is ready to accept (detected) a new connection.
                 * SocketChannel's registered with
                 * the `selector should not appear here.
                 */
                if (key.isAcceptable()) {
                    /*
                     * `listeningSC` is not a "new" channel - it is one of the ServerSocketChannels
                     * we registered
                     * with the selector in the `initializeChannels` method.
                     */
                    ServerSocketChannel listeningSC = (ServerSocketChannel) key.channel();
                    // We use a `SocketChanel` for established connections.
                    // `newChannel.isAcceptable()` will ALWAYS return False
                    SocketChannel newChannel = listeningSC.accept();
                    // All requests on this channel will be sent to "non-blocking"
                    newChannel.configureBlocking(false);
                    // Using identity comparison to determine which channel this is and act as a
                    // switch statement:
                    // Currently, the only connections that should be persistent are those between
                    // addressing server processes.
                    if (listeningSC == clientListenerChannel) {
                         ...
                         */
                        newChannel.register(selector, SelectionKey.OP_READ); // SocketChannel set to persistent
                                                                             // non-blocking I/O
                        registerReplicaServer(newChannel);
                        // The Primary `AddressingServer` maintains a persistent connection to replicas.
                        // Store the channel in an array.
                        this.replicaChannels.add(newChannel);
                    } else if (listeningSC == chatServerListenerChannel) {
                        try {
                            registerChatServer(newChannel);
                            // TODO: Share the entire {@code addressLog} with chat servers when they
                            // register.
                        } catch (IOException ioe) {
                 ...
                 * (as we are doing above with `newChannel`) will return True when invoking
                 * `isReadable()`.
                 * Typically, only a SocketChannel would ever be registered with OP_READ.
                 */
                if (key.isReadable()) {
                    /*
                     * Since replica servers are the only persistent connections, the logic
                     * contained
    ...
    public static void main(String[] args) {
        ...
        // ... without it, the initial output to console occurs before the terminal is
        // open
        try {
    ...
    /**
     * Logs the details of this ChatServerInfo object to the console for debugging
     * purposes.
     */
