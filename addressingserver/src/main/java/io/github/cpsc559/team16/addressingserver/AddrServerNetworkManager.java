...
public class AddrServerNetworkManager {
    ...
    public void openListenerChannels(int clientPort, int peerPort, int chatServerPort) {
        try {
            ...
            // TODO - Could add failure handling here, or just exit the process and spin up
            // a new container.
        }
    }
    ...
    // * @return {@code true} if the channel is known to be persistent (i.e.,
    // belongs to a peer or chat server), {@code false} otherwise.
    // */
    ...
    // * to find the {@code NIOMessageChannel} corresponding to the provided {@link
    // SocketChannel}.
    // * </p>
    // * <p>
    // * If the channel is not found in either manager, the method returns {@code
    // null}.
    // * </p>
    // *
    ...
    // return chatServerManager.getChannels().get(channel); // Will return null if
     ...
     * @param key     the {@code SelectionKey} associated with the channel, used for
     *                deregistration.
     ...
     */
    // private void cleanupPersistentConnection(SocketChannel channel, SelectionKey
    // key, Boolean cce) {
...
////        );
    ...
    // try {
    // channel.close();
    // } catch (IOException ignored) {} // if the channel is already closed, we
    // don't need to do anything.
    // }

    /**
     * Retrieves the internal {@link Selector} used for multiplexing non-blocking
     * I/O operations.
     * <p>
     * The {@code Selector} enables the {@code AddressingServer} to monitor multiple
     * registered
     * {@link SocketChannel}s and {@link ServerSocketChannel}s for events such as
     * connection
     * requests or available data. This method provides access to the selector for
     * components
     * that need to register new channels or monitor channel readiness (e.g., read
     * or accept).
     * </p>
     *
     * @return the internal {@code Selector} used by this
     *         {@code AddrServerNetworkManager}.
     */
     ...
     * </ul>
     * <p>
     * This method blocks until an event occurs on a registered channel (i.e., a
     * connection request
     * is received). When an event is detected, it retrieves the corresponding
     * SelectionKey,
     * processes the event, and removes the key(event) from the selector to prevent
     * re-processing.
     * </p>
     *
